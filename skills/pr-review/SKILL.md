...
挑完**不为凑数硬找** — 挑不出 = ✅ 合 + 指着具体决策致谢。假发现比无发现更伤作者信任。详 `REFERENCE.md` §1-3。

## 3. 给 user 反馈 + 发出去

每个发现给作者**两条路他自己选** + 严重度(🔴 阻塞 / 🟡 建议改 / 🟢 顺手提)。终端报告**一段话连着写** — 整体判断 + 关键发现 + 建议动作。**全部 inline,不写 file**。合并必配致谢(指着具体代码或决策说话)。不暴露内部账本(不列检查清单 / 不写"反问了三遍"过程信息),不凑数。评论是替 user 代笔(用"我"/"你",不露 skill 身份),3 条说话纪律见 `SHARED-COMMENT-VOICE.md`。
