...
## 2. 评估 issue(心里狠 ≠ 嘴上狠 — 心里挑剔无上限,嘴上对作者温和)

挑毛病不止盯着"字段填得全不全",先往**上一层**问:

- **真需求吗** — 真痛点,还是 user 一时兴起 / 已经有更简单替代?
- **更根本的解法吗** — 这条 issue 提的处理思路是不是绕远路?架构层面有没有更直接的解?
- **拆不该拆 / 合不该合** — scope 太大该拆?跟 N 条 open / placeholder 是同一件事该合?

然后**机械扫 5 维度 + 7 盲区**(详 `REFERENCE.md`):clarity / template 完整度 / 唯一性 / 可执行性 / 跟项目方向一致 + 7 盲区(描述精确 / 重复 / 必填字段 / 验证标准 / 方向冲突 / scope / 实物证据)。心里挑剔像内科医生听诊器从锁骨听到脐下每个位置都听一遍。

挑完**不为凑数硬找** — 挑不出 = ✅ approve + 指着具体决策致谢。假发现比无发现更伤作者信任。

## 3. 给 user 反馈 + 发出去

每个发现给作者**两条路他自己选** + 严重度(🔴 必须改 / 🟡 建议改 / 🟢 顺手提)。

挑 **1 个建议动作**(必须是这 5 种之一,详 `REFERENCE.md`):✅ approve + 致谢 / 🟡 完善 / 🔧 升级拆合并 / ⏸ 暂缓 user 拍板 / ❌ 关闭。**Approve 必配致谢评论**(指着具体决策 / 字段说话,不空 approve)。
